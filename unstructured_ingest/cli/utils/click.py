...
class DelimitedString(click.ParamType):
    ...
    def __init__(self, delimiter: str = ",", choices: Optional[list[str]] = None):
        ...
        self._choices_set = set(self.choices) if self.choices else None  # Add set for fast lookup
    ...
    def convert(
        ...
        choices_set = self._choices_set  # Cache attribute/field lookup
        for s in split:
            if s not in choices_set:
                self.fail(
                    ngettext(
