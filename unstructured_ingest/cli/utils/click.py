...
def extract_config(flat_data: dict, config: Type[BaseModelT]) -> BaseModelT:
    ...
    # Avoid unnecessary re-setting if model_config is already correct
    if getattr(config, "model_config", None) != {"extra": "ignore"}:
        config.model_config = ConfigDict(extra="ignore")
    if not flat_data:
        return config.model_validate(obj={})

    # Precompute field names for faster membership testing
    field_names_set = set()
    aliases = {}  # map from real field name to alias
    for k, v in fields.items():
        alias = v.alias or k
        field_names_set.add(alias)
        aliases[k] = alias

    # Filter flat_data in a single pass
    ...
    if access_config := fields.get("access_config"):
        ...
        ac_field_names_set = set()
        for k, v in ac_fields.items():
            ac_field_names_set.add(v.alias or k)
        access_config_data = {
